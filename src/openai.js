...
function buildInstructions() {
  return [
    ...
    'Make the target competency clear through the action, technical detail, and result rather than naming it as a category.',
    'Start every experience and project bullet directly with a concrete action, decision, or outcome. Do not prepend a heading-style label followed by a colon, such as "API integration:", "Scalable data modeling:", or "Responsive user experiences:".',
    'Vary sentence openings naturally across adjacent bullets so the resume reads as if an engineer wrote it, not as a taxonomy of competencies.',
...
function buildUserPrompt({ jd, bulletBank, tokenLimits, tokenMinimums, company, role }) {
  ...
  return [
    ...
    'requirements that the candidate can genuinely support. Give each bullet a distinct',
    'purpose internally, then express that purpose through the work itself rather than a',
    'category label. Write the most relevant bullet first and use source facts as evidence',
    'for it. Do not begin from a source sentence and edit it.',
    'Begin experience and project bullets with the concrete action, decision, or outcome;',
    'never prefix them with a short topical heading and a colon. Make the first clause',
    'immediately sound relevant to the target function. Prefer the',
...
function buildRevisionPrompt(overLimit, underLimit) {
  ...
  return [
    ...
    'Never add filler or facts absent from the context, do not revert to verbatim source text,',
    'and do not introduce a heading-style label followed by a colon.',
    ...shortenLines,
    ...expandLines,
  ].join('\n');
}
